...
| `reconnect` | Пересканировать сеть и переподключиться к найденным ПК |
| `saveips` | Сохранить IP-адреса подключённых ПК в файл |
| `help` | Список команд |
...
>> reconnect         # пересканировать сеть и переподключиться
>> saveips           # сохранить IP в файл room_403_ips.txt
>> exit              # выход
```
...
Используйте скрипт `scripts/setup_keys.sh` для автоматического копирования ключей на все ПК:

```bash
# Сохранить IP-адреса кабинета
>> saveips

# Распределить ключи
./scripts/setup_keys.sh -f room_403_ips.txt
```
...
|------|----------|--------------|
| `-f <file>` | Файл с IP-адресами | Обязательный |
| `-u <user>` | SSH пользователь | `teacher` |
| `-k <path>` | Путь к публичному ключу | `~/.ssh/classroom_agent.pub` |
